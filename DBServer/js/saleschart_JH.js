    ...
    options: {
        ...
        onClick: function clickHandler(evt) {           //sm. 클릭으로 이벤트 실행하기 바 차트의 바를 클릭해서 그 바의 값을 가져오고, 모달을 실행하고자함   
            ...
            if (points.length) {
                ...
                var myModal1 = new bootstrap.Modal(document.getElementById('JanuaryModal'))  //sm. 차트 클릭이벤트와 함께 모달 실행, 모달의 인스턴스 생상하여 모달의 아이디를 추적하여 저장
...
window.onload = function(){
    sendAjax2021('http://localhost:3005/sales.html');
}
...
function yearSelect(year) {
    switch (year) {
        ...
        case "2020":
            sendAjax2020("http://localhost:3005/sales.html");
            break;

        case "2019":
            sendAjax2019("http://localhost:3005/sales.html");
            break;
